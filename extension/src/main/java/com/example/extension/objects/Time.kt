...
val nowInNano: Long get() = System.nanoTime()
...
fun getDifferenceHour(startDate: Long, endDate: Long): Long {
    return TimeUnit.HOURS.convert(endDate - startDate, TimeUnit.MILLISECONDS)
}

fun getDifferenceDay(startDate: Long, endDate: Long): Long {
    return TimeUnit.DAYS.convert(endDate - startDate, TimeUnit.MILLISECONDS)
}

fun getDifferenceMinutes(startDate: Long, endDate: Long): Long {
    return TimeUnit.MINUTES.convert(endDate - startDate, TimeUnit.MILLISECONDS)
}

fun convertLongToTime(time: Long, locale: Locale, formatDateTime: String): String {
    val date = Date(time)
    val format = SimpleDateFormat(formatDateTime, locale)
    return format.format(date)
}

// Use:
//    convertLongToTime(timeStampSaved,context.resources.configuration.locales[0])
...
@SuppressLint("SimpleDateFormat")
fun formatDateFromPattern(
    input: String,
    pattern: String? = "yyyy-MM-dd'T'HH:mm:ss.SSSXXX"
): String {
    return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
        val formatterInput = DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss.SSSXXX")
...
//        val sdf = SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss")
        val sdf = if (pattern == "yyyy-MM-dd'T'HH:mm:ss.SSSXXX") {
